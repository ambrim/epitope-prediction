...
args = TrainingArguments(
    f"{model_name}-finetuned-epitope_attempt7_150M_200k_weighted_accuracy_3", # 93% 0, 7$ 1
...
class CustomTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        loss_fct = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([3.0], device=device))

        # loss_fct = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([3.0], device=model.module.device))
        
        # Apply sigmoid to logits and reshape labels to match logits shape
        loss = loss_fct(logits.reshape(-1), labels.float().reshape(-1))
...
df.to_csv('LOG_attempt7_150M_200k_weighted_accuracy_3_before.csv', index=False)

trainer.train()

# Evaluate the model
pd.DataFrame(trainer.state.log_history)
df.to_csv('LOG_attempt7_150M_200k_weighted_accuracy_3.csv', index=False)
